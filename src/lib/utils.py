...
def animate_pose_sequence(
    ...
    for j in anim_range:
        ...
        if annotation_sequence is not None:
            ...
            plt.text(
                50, -20, annot, fontsize=16, bbox=dict(facecolor=class_to_color[annot], alpha=0.5)
            )
...
def make_sequences(sub_clips, train_data):
    ...
    for key, value in train_data["sequences"].items():
        ...
        train_sequences.append(Sequence(key, value["keypoints"], labels[0], labels[1][0]))
