...
def mice_distance_angle(
    ...
    """
    mice = [
        Sequence.mouse_from_frame(frame, 0),
...
def calc_mouse_length(frame: np.ndarray) -> np.ndarray(dtype=float, shape=(3)):
    ...
    for m in range(3):
        lengths[m] = np.linalg.norm(
            Sequence.name_mouse(mice[m])["nose"] - Sequence.name_mouse(mice[m])["tail base"]
        )
    return lengths
...
def calc_mean_reach(frame: np.ndarray) -> np.ndarray(dtype=float, shape=(3)):
    """
    calculates the mean distance from keypoints to 'center back' for each mouse (excluding the
    tail)
    :param frame: original keypoints - 72 numbers
    :return: float array
    """
    ...
    for m in range(3):
        ...
        for keypoint in Sequence.name_mouse(mice[m]):
            if keypoint not in ["center back", "tail middle", "tail tip"]:
                distances.append(
                    np.linalg.norm(
                        Sequence.name_mouse(mice[m])[keypoint]
...
def calc_head_angle(frame: np.ndarray) -> np.ndarray(dtype=float, shape=(3)):
    ...
    for m in range(3):
        ...
        if body_direction_norm != 0 and head_direction_norm != 0:
            dot_product = (
                np.dot(body_direction, head_direction) / body_direction_norm / head_direction_norm
            )
            dot_product = np.clip(dot_product, -1, 1)
            angle = np.arccos(dot_product)
            result[m] = angle
