...
def submission_embeddings(config: dict, sub_clips: dict, model: nn.Module, sub_seq: list, func = None, embd: list = None) -> dict:
    ...
    Creates a ready for submission dict with an arbitrary model (not true yet).
    ...
    :return: dict ready for submission (however should be validated with the validate_submission function before using it)
    """
    ...
    for sequence_key in tqdm(sub_clips["sequences"]):
        ...
        embs = model(torch.Tensor(X).cuda().unsqueeze(0), return_embeddings = config["return_embeddings"])[0] # probably works only for the perceiver for now

        for i in range(len(keypoints)):

            if i % config["freq_embd_calc"] == 0 and i + seq_len < len(keypoints): # in the initial notebook config["freq_embd_calc"] is 100, ideally should be 1 but really long submission time
                X = np.array([keypoints[i].flatten() for i in range(0, seq_len, config["frame_increment"])])
                embs = model(torch.Tensor(X).cuda().unsqueeze(0), return_embeddings = config["return_embeddings"])[0] # probably works only for the perceiver for now
