...
def gif(
    ...
    for file in files:
        ...
        try:
            ...
            np.save(
                os.path.join(path_to_file, "community", "umap_embedding_" + file + ".npy"), embed
            )
        ...
        frames = get_animal_frames(
            cfg, file, pose_ref_index, start, length, subtract_background, file_format, crop_size
        )

        create_video(
            path_to_file, file, embed, umap_label, frames, start, length, max_lag, num_points
        )
