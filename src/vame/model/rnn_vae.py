...
def train_model(config, live_logging=False):
    ...
    for epoch in range(1, EPOCHS):
        ...
        np.save(
            os.path.join(
                cfg["project_path"], "model", "model_losses", "train_losses_" + model_name
            ),
            train_losses,
        )
        np.save(
            os.path.join(cfg["project_path"], "model", "model_losses", "test_losses_" + model_name),
            test_losses,
        )
        np.save(
            os.path.join(
                cfg["project_path"], "model", "model_losses", "kmeans_losses_" + model_name
            ),
            kmeans_losses,
        )
        np.save(
            os.path.join(cfg["project_path"], "model", "model_losses", "kl_losses_" + model_name),
            kl_losses,
        )
        np.save(
            os.path.join(
                cfg["project_path"], "model", "model_losses", "weight_values_" + model_name
            ),
            weight_values,
        )
        np.save(
            os.path.join(
                cfg["project_path"], "model", "model_losses", "mse_train_losses_" + model_name
            ),
            mse_losses,
        )
        np.save(
