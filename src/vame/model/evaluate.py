...
def plot_reconstruction(
    ...
    if use_gpu:
        ...
        data_fut = (
            x[:, seq_len_half : seq_len_half + FUTURE_STEPS, :].type("torch.FloatTensor").cuda()
        )
    else:
        data = x[:, :seq_len_half, :].type("torch.FloatTensor").to()
        data_fut = (
            x[:, seq_len_half : seq_len_half + FUTURE_STEPS, :].type("torch.FloatTensor").to()
        )
    if FUTURE_DECODER:
    ...
    if FUTURE_DECODER:
        ...
        for i in range(5):
            ...
            axs[0, i].plot(
                data_tilde[i, ...], color="r", linestyle="dashed", label="Sequence Reconstruction"
            )
